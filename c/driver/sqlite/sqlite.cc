...
struct SqliteGetObjectsHelper : public driver::GetObjectsHelper {
  ...
  Status Load(int depth, std::optional<std::string_view> catalog_filter,
              ...
              // TODO: const ref
              std::vector<std::string_view> table_types) override {
    std::string query =
  ...
  Status LoadCatalogs() override { return status::kOk; };
  ...
  Status LoadTables(std::string_view catalog, std::string_view schema) override {
    next_table = 0;
    tables.clear();
    if (!schema.empty()) return status::kOk;
 ...
 public:
  ...
  Status SetOptionImpl(std::string_view key, driver::Option value) override {
    if (key == "uri") {
      ...
      if (!uri.has_value()) {
        return status::InvalidArgument("uri must not be NULL");
      }
      uri_ = std::move(*uri);
      return status::kOk;
    }
 ...
 public:
  ...
  Status SetOptionImpl(std::string_view key, driver::Option value) {
    if (key == kConnectionOptionEnableLoadExtension) {
      ...
      UNWRAP_RESULT(auto path, value.AsString());
      if (!path) {
        return status::InvalidArgument("extension path must not be NULL");
      }
      extension_path_ = std::move(*path);
      return status::kOk;
      ...
      UNWRAP_RESULT(auto maybe_entrypoint, value.AsString());
      const char* extension_entrypoint =
          maybe_entrypoint ? maybe_entrypoint->c_str() : nullptr;
...
// TODO: this should be templated out via macro
